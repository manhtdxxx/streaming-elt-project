...
WITH source AS (
    SELECT * 
    FROM {{ source('raw_taxi', 'green_taxi') }}
    {% if is_incremental() %}
...
after_flagged AS (
    SELECT *,
        ...
        -- ehail_fee flag (green taxi only)
        CASE
            WHEN ehail_fee IS NULL THEN 'missing'
            WHEN ehail_fee < 0 THEN 'invalid'
            WHEN ehail_fee = 0 THEN 'zero'
            ELSE 'valid'
        END AS ehail_fee_flag,
        ...
        -- trip_type flag (green taxi only)
        CASE
            WHEN trip_type IS NULL THEN 'missing'
            WHEN trip_type NOT IN (1, 2) THEN 'invalid'
            ELSE 'valid'
        END AS trip_type_flag
